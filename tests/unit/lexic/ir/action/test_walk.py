...
Action resolution
-----------------
``actions`` is an :class:`~lexic.ir.action.mapping.IrTypeMap` of
...
``IrVisitor``       Side-effect walker. Default action :class:`~lexic.ir.action.IrWalk`
                    recurses into children and returns :data:`~lexic.ir.grammar.nodes.IrNone`.
``IrTransformer``   Rewrites IR. Default action
                    :class:`~lexic.ir.action.IrRebuild` walks children via
                    ``d`` and rebuilds the node.
``IrEmitter``       Produces :class:`~lexic.ir.grammar.nodes.IrLiteral`. Default
                    action :class:`~lexic.ir.action.IrEmit` wraps ``str(n)``
                    in :class:`~lexic.ir.grammar.nodes.IrLiteral`; override with
...
def test_irdispatch_apply_with_no_actions_invokes_preset_default():
    ...
    returns :data:`~lexic.ir.grammar.nodes.IrNone`."""
