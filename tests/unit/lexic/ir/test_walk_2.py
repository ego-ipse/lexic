...
The dispatcher is an :class:`~lexic.ir.nodes_2.IrComposite` whose ``actions``
...
cares about, or by leveraging :class:`~lexic.ir.action_2.IrRebuild` which
walks then rebuilds).

Action resolution
-----------------
The matching :class:`~lexic.ir.action_2.IrAction` is resolved via
...
is :class:`~lexic.ir.action_2.IrRaise`, that raises
:exc:`~lexic.exceptions.UnsupportedConstructError`.

Short-circuit
-------------
A body raising :class:`~lexic.ir.action_2.IrReturn` is caught at ``apply``
...
bare :class:`~lexic.ir.action_2._Return`).

Presets
-------
``IrVisitor``       Side-effect walker. Default action :class:`~lexic.ir.action_2.IrWalk`
                    recurses into children and returns :data:`~lexic.ir.nodes_2.IrNone`.
``IrTransformer``   Rewrites IR. Default action
                    :class:`~lexic.ir.action_2.IrRebuild` walks children via
                    ``d`` and rebuilds the node.
``IrEmitter``       Produces :class:`~lexic.ir.nodes_2.IrLiteral`. Default
                    action :class:`~lexic.ir.action_2.IrEmit` wraps ``str(n)``
                    in :class:`~lexic.ir.nodes_2.IrLiteral`; override with
...
# NOTE: test_irdispatch_is_an_ircollection_of_actions REMOVED.
# In the primitive-node model IrDispatch IS-AN IrComposite (not IrCollection);
# IrCollection no longer exists in nodes_2; _child_attrs defaults to () so
# children() returns () — the actions tuple is NOT a structural child of the
# dispatcher. The test targeted both a deleted class (IrCollection) and deleted
# behaviour (children() returning the actions). Both are gone by design.


def test_irdispatch_is_composite():
    """IrDispatch IS-AN IrComposite — not IrCollection (removed in primitive model)."""
...
def test_irdispatch_apply_with_no_actions_invokes_preset_default():
    """``IrVisitor``'s preset default :class:`~lexic.ir.action_2.IrWalk`
    returns :data:`~lexic.ir.nodes_2.IrNone`."""
