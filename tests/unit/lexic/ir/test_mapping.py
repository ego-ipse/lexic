...
# ── at(): the typed nested path read ─────────────────────────────────────


def _nested() -> IrMap:
    """{"model": {"vocab": {"a": 1}, "n": 2}} as nested IrMaps."""
    vocab = IrMap(IrTuple(IrStr("a"), IrInt(1)))
    model = IrMap(IrTuple(IrStr("vocab"), vocab), IrTuple(IrStr("n"), IrInt(2)))
    return IrMap(IrTuple(IrStr("model"), model))


def test_at_walks_a_nested_path():
    """at() chains __getitem__ down nested maps in one call."""
    assert _nested().at("model", "n") == 2


def test_at_single_key_equals_getitem():
    """A one-key at() is exactly the subscript read."""
    doc = _nested()
    assert doc.at("model") is doc["model"]


def test_at_into_checks_and_returns_the_type():
    """into= asserts the endpoint's type as part of the read contract."""
    vocab = _nested().at("model", "vocab", into=IrMap)
    assert vocab["a"] == 1


def test_at_raises_with_the_failing_path_on_a_miss():
    """An absent key raises IrKeyError naming the walked prefix."""
    with pytest.raises(IrKeyError, match=r"at model\.absent"):
        _nested().at("model", "absent")


def test_at_raises_on_a_non_mapping_interior():
    """Descending through a leaf raises with the interior path."""
    with pytest.raises(UnsupportedConstructError, match=r"at model\.n"):
        _nested().at("model", "n", "deeper")


def test_at_raises_on_an_into_mismatch():
    """A wrong endpoint type raises rather than returning mistyped."""
    with pytest.raises(UnsupportedConstructError, match="expected IrMap"):
        _nested().at("model", "n", into=IrMap)
