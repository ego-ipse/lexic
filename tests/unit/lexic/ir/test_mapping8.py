"""Tests for ``ir/mapping8.py`` — the common-ancestor fast map family.
...
equality) without the tuple-ancestry artefacts (positional ``m[int]``/slice,
per-table synthesized classes). The two concrete maps are siblings under
:class:`~lexic.ir.mapping8.IrMapping`; :class:`~lexic.ir.mapping8.IrMultiMap`
reads return the **live** bucket (no snapshot copy).
"""
...
from lexic.ir.mapping8 import IR_DEFAULT, IrMap, IrMapping, IrMultiMap, IrTypeMap
...
def test_irtype_map_miss_raises():
    ...
    with pytest.raises(UnsupportedConstructError):
        IrTypeMap(IrTuple(IrLiteral, IrThis())).eval(None, IrRuleRef("r"), ())
...
def test_multimap_files_and_reads_live_bucket():
    ...
    assert bucket is mm._table[IrStr("k")]  # live, not a snapshot copy
...
def test_multimap_table_slot_available_for_subclasses():
    ...
    mm: IrMultiMap = IrMultiMap()
    mm += (IrStr("k"), IrStr("v"))
    assert mm._table == {IrStr("k"): [IrStr("v")]}
