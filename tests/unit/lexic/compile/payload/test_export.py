...
def test_the_artefact_records_where_each_symbol_came_from() -> None:
    ...
    assert "lexic.ir.spine.records" in source
...
def test_a_home_is_the_origin_only_when_the_origin_really_exports_it() -> None:
    """Identity, not presence — ``lexic.ir.spine.records`` merely IMPORTS ``Sequence``.
    ...
    """
    impostor = type("Sequence", (str,), {"__module__": "lexic.ir.spine.records"})
