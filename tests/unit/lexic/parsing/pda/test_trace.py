...
from lexic.parsing import PdaKernel, Step
...
def test_a_traced_parse_records_every_clone_it_enters() -> None:
    ...
    compiled = compile_text(_TOY)
    steps: list[Step] = []
    PdaKernel(compiled.pda_tables(), "ab", compiled.fold, trace=steps).run()
    assert steps
...
def test_the_trace_names_the_build_mode_of_each_clone() -> None:
    ...
    compiled = compile_from_path(GROUND_TRUTH / "json.gbnf")
    steps: list[Step] = []
    PdaKernel(compiled.pda_tables(), '{"a":1}', compiled.fold, trace=steps).run()
...
def test_tracing_does_not_change_what_a_parse_produces() -> None:
    ...
    steps: list[Step] = []
    traced = PdaKernel(compiled.pda_tables(), text, compiled.fold, trace=steps).run()
    assert plain == traced
...
def test_a_refused_parse_still_records_what_it_managed() -> None:
    ...
    with pytest.raises(Exception):
        PdaKernel(compiled.pda_tables(), "az", compiled.fold, trace=steps).run()
    assert steps
