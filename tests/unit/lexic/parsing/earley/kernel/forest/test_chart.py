"""Tests for lexic.parsing.earley.kernel.forest.chart — Chart, Link, Links.

API changes from the int-kernel rework:

- ``Column`` is COMPLETELY GONE. The per-column Earley sets now live in the
  compiled kernel (:mod:`lexic.parsing.earley.kernel.loop.kernel`), packed as ints. All
...
- ``Chart()`` no longer takes an index and there is no ``chart[i]`` access.
  ...
  populated by :func:`~lexic.parsing.earley.kernel.forest.support.readout.to_chart`.
...
from lexic.parsing.earley.kernel.forest.chart import Chart, EarleyItem, Links

# ── Helpers ─────────────────────────────────────────────────


def make_arm(*chars: str) -> IrSequence:
    """Build an IrSequence of single-char IrLiteral items."""
    return IrSequence(*(IrItem(IrLiteral(c)) for c in chars))


def ei(rule: str, arm: IrSequence, dot: int = 0, origin: int = 0) -> EarleyItem:
    """An EarleyItem tuple for ``rule``, at ``dot`` over ``arm``, from ``origin``."""
...
def make_item(arm: IrSequence, dot: int = 0, origin: int = 0) -> EarleyItem:
    """An EarleyItem tuple for the fixture rule ``"s"``."""
...
def test_links_multi_family_two_distinct_links():
    ...
    arm = make_arm("x")
    item = ei("s", arm, dot=1)
    pred_a = ei("s", arm, dot=0)
    pred_b = ei("r", arm, dot=0)
    child = IrLiteral("x")
...
def test_items_differ_by_dot():
    ...
    arm = make_arm("a", "b")
    assert make_item(arm, dot=0) != make_item(arm, dot=1)
