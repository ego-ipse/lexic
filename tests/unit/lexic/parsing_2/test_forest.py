"""Tests for lexic.parsing_2.forest — ParseTree and build_tree."""
...
def test_build_tree_returns_parse_tree():
    """build_tree returns a ParseTree for a simple single-char input."""
    grammar = _make_digit_grammar()
    tree = EarleyParser().parse(grammar, "5")
...
def test_build_tree_symbol_is_start_rule():
    ...
    grammar = _make_digit_grammar()
    tree = EarleyParser().parse(grammar, "7")
...
def test_build_tree_kids_in_source_order():
    ...
    tree = EarleyParser().parse(grammar, "hi")
    assert isinstance(tree, ParseTree)
...
def test_build_tree_leaf_kids_are_ir_literals():
    ...
    grammar = _make_digit_grammar()
    tree = EarleyParser().parse(grammar, "3")
...
def test_build_tree_recursive_grammar_nests_correctly(expr_grammar: IrAst):
    ...
    tree = EarleyParser().parse(expr_grammar, "(5)")
