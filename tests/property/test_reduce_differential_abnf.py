...
The ABNF sibling of ``test_reduce_differential.py`` (GBNF) — read that file's
docstring first, this one only restates what's ABNF-specific. ``parse_reduced``
...
``ABNF_FLAVOUR``'s own self-grammar (the meta-grammar ABNF text is parsed
*against*):
...
a lifted-nullable reduction (e.g. an optional ``repeat-opt`` producing empty
children) agree with the unlifted route's node-skip. This file is the guard
that they actually do agree: PDA-recognised text must be a subset of
Earley-recognised text, and wherever both recognise, the reduced
:class:`IrAst` must be equal.

``ReduceDifferential.earley_grammar`` below is the flip point a later task
retargets to the lifted grammar for all three flavours' differentials at
once.
...
# normalize(lift_optional_nullables(ABNF_FLAVOUR.grammar)).
