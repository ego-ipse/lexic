...
def ast_to_regex(node: ASTNode, rules: dict[str, RuleNode]) -> str:
    ...
    if isinstance(node, RepetitionNode):
        ...
        needs_group = not (inner.startswith("[") or (len(inner) == 1 and inner not in r"\.^$*+?{}[]|()" ))
    ...
    for node in seq.nodes:
        ...
        if isinstance(node, RuleRefNode):
            ...
            if isinstance(inner, RuleRefNode):
                base = to_field_name(inner.name) if is_optional else pluralise(to_field_name(inner.name))
...
def _gen_nonterminal_parser(rule: RuleNode, rules: dict[str, RuleNode]) -> str:
    ...
    if isinstance(body, RuleRefNode):
        if body.target and body.target.rule_is_terminal:
            ...
            lines.append(f"    raise ParseError({fname!r}, pos, text)  # unresolved ref")
        ...
        if is_list:
            ...
            if isinstance(inner, SequenceNode):
                ...
                lines.append("        try:")
                collected: list[str] = []
                _emit_sequence_body(inner, lines, fname, "            ", rules, collected)
                ...
                if inner.target.rule_is_terminal:
                    ...
                    lines.append(f"            value.append(_item)")
                    lines.append(f"        except ParseError: break")
                else:
                    lines.append(f"    while True:")
                    lines.append(f"        try:")
                    lines.append(f"            _item, pos = parse_{inner.target.name}(text, pos)")
                    lines.append(f"            value.append(_item)")
                    lines.append(f"        except ParseError: break")
            lines.append(f"    return {cname}(value=value), pos")
        else:
            lines.append(f"    raise ParseError({fname!r}, pos, text)  # unhandled repetition")
            ...
            if isinstance(node, LiteralNode):
                ...
                if is_optional:
                    if isinstance(inner, RuleRefNode) and inner.target:
                        var = to_field_name(inner.target.name)
                        lines.append(f"    try:")
                        if inner.target.rule_is_terminal:
                            lines.append(f"        {var}, pos = _parse_{inner.target.name}(text, pos)")
                        else:
                            lines.append(f"        {var}, pos = parse_{inner.target.name}(text, pos)")
                        lines.append(f"    except ParseError: pass")
                        if var not in field_vars:
                            field_vars.append(var)
                    elif isinstance(inner, SequenceNode):
                        lines.append(f"    _opt_pos = pos")
                        lines.append(f"    try:")
                        _emit_sequence_body(inner, lines, fname, "        ", rules, field_vars)
                        lines.append(f"    except ParseError: pos = _opt_pos")

                elif is_list:
                    if isinstance(inner, RuleRefNode) and inner.target:
                        var = pluralise(to_field_name(inner.target.name))
                        lines.append(f"    while True:")
                        lines.append(f"        try:")
                        if inner.target.rule_is_terminal:
                            lines.append(f"            _item, pos = _parse_{inner.target.name}(text, pos)")
                        ...
                        for n in inner.nodes:
                            if isinstance(n, RuleRefNode) and n.target:
                                list_var = pluralise(to_field_name(n.target.name))
                                break
                        if list_var is None:
                            list_var = "items"
                        lines.append(f"    while True:")
                        lines.append(f"        _loop_pos = pos")
                        lines.append(f"        try:")
                        tmp: list[str] = []
                        _emit_sequence_body(inner, lines, fname, "            ", rules, tmp)
                        for n in inner.nodes:
                            if isinstance(n, RuleRefNode) and n.target:
                                item_var = to_field_name(n.target.name)
                                lines.append(f"            {list_var}.append({item_var})")
                                break
                        lines.append(f"        except ParseError:")
                        lines.append(f"            pos = _loop_pos")
                ...
                if node.target.rule_is_terminal:
                    ...
                    lines.append(f"    {var}, pos = parse_{node.target.name}(text, pos)")
                if var not in field_vars:
                    field_vars.append(var)
...
def _emit_sequence_body(
    ...
    for node in seq.nodes:
        if isinstance(node, LiteralNode):
            ...
            var = to_field_name(node.target.name)
            if node.target.rule_is_terminal:
                lines.append(f"{indent}{var}, pos = _parse_{node.target.name}(text, pos)")
            else:
                lines.append(f"{indent}{var}, pos = parse_{node.target.name}(text, pos)")
