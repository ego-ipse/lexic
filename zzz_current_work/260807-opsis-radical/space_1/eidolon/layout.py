...
from eidolon.topology import levels
from lexic.ir import IrAst

__all__ = ["VIEWS", "positions"]
...
def flat(ast: IrAst, wide: int, tall: int) -> dict[str, tuple[float, float, float]]:
    ...
    for at, names in bands.items():
        ...
        row_tall = min(float(ROW), max(14.0, (tall - 40) / max(1, rows)))
...
def arcs(ast: IrAst, wide: int, _tall: int) -> dict[str, tuple[float, float, float]]:
    """One row, in the order the grammar declares its rules."""
    names = [str(rule.name) for rule in ast.rules]
    pitch = max(18.0, (wide - 40) / max(1, len(names)))
    return {name: (20 + i * pitch, 0.0, 0.0) for i, name in enumerate(names)}


def rings(ast: IrAst, _wide: int, _tall: int) -> dict[str, tuple[float, float, float]]:
    """A ring per level in 3-space — z is derivation distance, the earned axis."""
    out: dict[str, tuple[float, float, float]] = {}
    for at, names in _bands(ast).items():
        count = len(names)
        radius = 0.0 if count == 1 else 46 + min(230, count * 15)
        for i, name in enumerate(names):
            angle = (i / count) * pi * 2 + at * 0.7
            out[name] = (cos(angle) * radius, sin(angle) * radius, -at * 140.0)
    return out
...
def positions(
    ast: IrAst, view: str = "rings", wide: int = 900, tall: int = 600
) -> dict[str, tuple[float, float, float]]:
    """Every rule's place, for one way of looking at the grammar."""
    return LAYOUTS.get(view, rings)(ast, wide, tall)
