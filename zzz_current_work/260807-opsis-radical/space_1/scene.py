...
on this side: what atlas hardcoded per fixture, the relation model answers.
The ladder is the strip DERIVED from the graph's edges, and the rule graph
comes from the reader's own AST rather than from anything declared here.
"""
...
def ladder(session: Session, rid: str) -> list[str]:
    """The strip: the path that led here, then the casts not yet taken.

    Derived every time from the edges and from what the engine licenses —
    there is no list of rungs to fall out of date.
    """
    out = []
    walked = list(session.strip(rid))
    for place, step in enumerate(walked):
        lit = 1 if step == rid else 0
        out.append(f"{place} {lit} r {session.relations[step].label()}")
    for offer in session.moves(rid):
        if offer.role.name != DOCUMENT.name or not offer.thing.spelling():
            continue
        out.append(f"{len(out)} 0 x {offer.thing.name} as {offer.role.name}")
    return out
...
def scene(session: Session, rid: str) -> str | None:
    ...
    if not isinstance(relation, Reading):
        return None
    reader = relation.cast[READER.name]
    reader_text = reader.spelling() or _spelling(relation)
    document = relation.cast[DOCUMENT.name].spelling()
...
def _spelling(relation: Reading) -> str:
    """A value reader has no text of its own — spell it through its flavour."""
    facets = [f for f in relation.facets() if f.name == "reader"]
    return getattr(facets[0], "text", "") if facets else ""
