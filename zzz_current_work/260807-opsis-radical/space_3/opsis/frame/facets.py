...
from opsis.frame.tones import ADVANCE, runs
...
def graph(said: Frame, room: Room, look: Look) -> None:
    ...
    # A graph pin's own reference header carries its view selector. Space 1
    # does not repeat the main facet's tuning tray inside that compact window.
    if look.says("window.kind", "") != "clone":
...
def rail(said: Frame, room: Room, look: Look) -> None:
    ...
    if not drawn.marks:
        ...
        return
    # The reference fits BOTH dimensions, permits a modest enlargement and
    # never shrinks a track into invisibility.
    scale = min(
        1.4,
        max(
            0.4,
            min((w - 16) / max(1.0, drawn.wide), (h - 16) / max(1.0, drawn.tall)),
        ),
    )
...
def _graph_text(said: Frame, room: Room, look: Look) -> None:
    """The graph pin's sixth view — the reader as real selectable text."""
    _plane(said, room, look, "relations-text", look.reading.reader_text, False)
...
def chart(said: Frame, room: Room, look: Look) -> None:
    ...
    said.report("chart.at", str(start))
...
def _rows(
    ...
    for gutter, words, extent, tone, goes in visible:
        ...
        if goes:
            ...
            if _pointed(goes) == hand:
                # Space 1's `.row:hover`: the spine row itself answers the
                # hand, in addition to the document and reader co-highlight.
                said.box(x + 8, top, w - 24, ROW, "liveline")
        if gutter:
...
def pin(said: Frame, room: Room, look: Look) -> None:
    ...
    stale = was.isdigit() and int(was) != look.generation
    name = look.says(
        f"pin.rule.{look.scope}",
        as_written(look.it.rules, span.rule) if span is not None else "",
    )
    field = look.says(f"pin.field.{look.scope}", span.field if span is not None else "")
    snippet = look.says(
        f"pin.text.{look.scope}", look.reading.text[s0 : min(e0, s0 + 400)]
    )
    definition = look.says(f"pin.def.{look.scope}", "")
    if not definition and span is not None and not stale:
        found = next(
            (
                (first, last)
                for rule, first, last in ruledefs(look.reading.reader_text)
                if rule == name
            ),
            None,
        )
        if found is not None:
            first, last = found
            definition = "\n".join(
                look.reading.reader_text.split("\n")[first : min(last + 1, first + 3)]
            )
    room_chars = max(8, int((w - 22) / ADVANCE["pinbody"]))
    lines = _wrapped(snippet + (" …" if e0 - s0 > 400 else ""), room_chars)
    cursor = y + 19
    for line in lines:
        said.text(x + 11, cursor, "ink", line, w - 22, face="pinbody")
        cursor += 14
    fact = (f"field {field} · " if field else "") + f"pinned against gen {was}"
    if stale:
        fact += " · the document has moved on — re-pin or close"
    cursor += 6
    fact_chars = max(8, int((w - 22) / ADVANCE["pinfact"]))
    for line in _wrapped(fact, fact_chars):
        said.text(x + 11, cursor, "dim", line, w - 22, face="pinfact")
        cursor += 13
    if not stale:
        source = definition.splitlines()
        if source:
            cursor += 6
        for line in source:
            for wrapped in _wrapped(line, room_chars):
                said.text(x + 11, cursor, "cool", wrapped, w - 22, face="pinbody")
                cursor += 14
    if name:
        said.hit(x, y, w, min(h, max(14, cursor - y)), "rule", name)
