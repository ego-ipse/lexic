...
class Frame:
    ...
    def __init__(self, wide: int, tall: int) -> None:
        ...
        # A semantic surface can occur more than once. Its window id scopes
        # browser identity and gesture routing without changing what the
        # surface means (`document`, `graph.view`, `top.graph`, ...).
        self._scope = ""
    ...
    @contextmanager
    def within(self, scope: str) -> Iterator[None]:
        """Address controls, planes and hits to one live window copy."""
        was = self._scope
        self._scope = scope
        try:
            yield
    ...
    def _address(self, name: str) -> str:
        """A browser identity carrying its window route, when it has one."""
        return f"{self._scope}~{name}" if self._scope else name

    def report(self, key: str, value: str) -> None:
        """Remember derived view state in the layer which produced it."""
        self.reported[self._address(key)] = value

    def open_layer(self, scope: str) -> None:
        """Begin one independently stacked visual window."""
        self._put(f"layer {scope}")

    def close_layer(self) -> None:
        """Finish the independently stacked visual window."""
        self._put("unlayer")
    ...
    def pick(
        ...
        self.picks.append(
            f"{self._address(key)} {x:.1f} {y:.1f} {w:.1f} {h:.1f} {chosen} {said}"
        )
    ...
    def slider(
        ...
        self.picks.append(
            f"{self._address(key)} {x:.1f} {y:.1f} {w:.1f} {h:.1f} "
            f"{now:g} range:{low:g}:{high:g}:{by:g}"
        )
    ...
    def hit(
        ...
        """
        self.hits.append(
            f"{x:.1f} {y:.1f} {w:.1f} {h:.1f} {kind} {self._address(goes)} "
            f"{run:.1f} {cell:.1f}"
        )
    ...
    def plane(
        ...
        """
        self.planes.append(
            f"{self._address(name)} {x:.1f} {y:.1f} {w:.1f} {h:.1f} "
            f"{ROW * zoom:.2f} "
