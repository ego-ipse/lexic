...
def fold(model: GrammarModel) -> tuple[str, list[Span]]:
    ...
    Reads the engine's own addressed emission (`GrammarModel.emit_addressed`)
    rather than re-deriving text geometry here: lexic knows the offsets and
    now hands them back, so this is a projection of an engine product into
    the five fields the rooms read — not a second walk that could drift from
    `to_text`.

    A `Span` is one MODEL occurrence, as before: `emit_addressed` also
    addresses literals and repeated-field tuples, and those are dropped here.
    `depth` counts model ancestors (a tuple layer is not a level), and `field`
    is the nearest name on the way up — the tuple carries the field name its
    elements were emitted under.
    """
    ...
    for extent in emission.extents:
        ...
        span = Span(
            extent.span.start,
            _model_depth(model, extent.address),
            type(occurrence).__grammar__.name,
            _named(extent.address),
        )
        span.end = extent.span.end
        spans.append(span)
    return emission.text, spans


def _model_depth(root: GrammarModel, address) -> int:
    """How many MODEL ancestors an address has — tuples are not a level."""
    return sum(
        1
        for cut in range(len(address))
        if isinstance(root.occurrence(address[:cut]), GrammarModel)
    )


def _named(address) -> str:
    """The nearest field name on the way up — "" at the root."""
    for step in reversed(list(address)):
        if step.field:
            return step.field
    return ""
