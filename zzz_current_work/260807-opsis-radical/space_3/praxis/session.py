...
from opsis.frame.tones import ADVANCE
...
from praxis.graph import Graph
from praxis.history import Retype, retype
from praxis.reading import Reading, as_written, read_up
...
class Session:
    ...
    def __init__(self, reading: Reading) -> None:
        ...
        # Storage is the graph. The ladder remains a compatibility projection
        # for the current strata wire, derived from rooted lineage edges.
        self.graph = Graph(reading)
        # Ambiguity is topology, not a warning: one subject with one offered
        # read relation per accepting metagrammar. They remain unheld until a
        # cast gesture chooses one, so probing does not secretly parse twice.
        for flavour in reading.offers if reading.ambiguous else ():
            offered = Reading(reading.reader, reading.document, flavour)
            offered.reader_name = reading.reader_name
            offered.reader_text = reading.reader_text
            offered.text = reading.text
            offered.offers = reading.offers
            offered.probes = reading.probes
            self.graph.add(offered, "offer")
        self.at = 0.0
        self.playing = False
        ...
        self.windows = 0
        self.generation = 1
        # Presentation belongs to the focused relation. Policy is the
        # session-wide fallback beneath it, so returning to a relation restores
        # its camera, arrangement and windows without copying global policy.
        self.policy: dict[str, str] = {}
        self.rooms: dict[str, dict[str, str]] = {self.graph.focus: {}}
        self.main: MutableMapping[str, str] = ChainMap(
            self.rooms[self.graph.focus], self.policy
        )
        ...
        self.body = ""
        # The compact windows are sized against the page they open in. The
        # frame request refreshes this before applying its gesture; this
        # default is also the gate's browser-sized room.
        self.viewport: tuple[int, int] = (1400, 800)

    @property
    def climbed(self) -> list[Reading]:
        """The lineage ladder expected by the wire, derived from the graph."""
        return self.graph.walked()
    ...
    def _set(self, words: list[str]) -> None:
        ...
        if key == "pin.span":
            ...
            return
        if key.startswith("rail.parent.") and value:
            self._railgo([f"{key.removeprefix('rail.parent.')}:{value}"])
            return
    ...
    def open_window(
        ...
        self.windows += 1
        # The reference has three cascades: general facet windows, occurrence
        # pins, and rail pins.
        if facet == "pin":
            x = 240 + (at % 8) * 44 + (at // 8) * 12
            y = 110 + (at % 8) * 44
        else:
    ...
    def _shut(self, words: list[str]) -> None:
        ...
        for key in [key for key in self.main if key.endswith(f".{wid}")]:
    ...
    def _window(self, words: list[str]) -> None:
        ...
        if words[1] == "size":
            w, h = str(max(280, int(w) + int(dx))), str(max(90, int(h) + int(dy)))
        else:
            x, y = str(max(0, int(x) + int(dx))), str(max(34, int(y) + int(dy)))
        self.main[f"win.{wid}"] = f"{facet} {x} {y} {w} {h} {about}"

    def _cast(self, words: list[str]) -> None:
        """Choose one offered reader relation; only this gesture parses it."""
        if not words or words[0] not in self.reading.offers:
            return
        flavour = words[0]
        held = Reading(self.reading.reader, self.reading.document, flavour)
        held.reader_name = self.reading.reader_name
        held.reader_text = self.reading.reader_text
        held.text = self.reading.text
        held.hold()
        self.enter(held)
        # The unresolved ingress relation was only a staging point. The cast
        # reading is now the rooted reading from which a lineage can climb;
        # the other offers remain in the graph as roads not taken.
        self.graph.root = self.graph.focus
    ...
    def _rung(self, words: list[str]) -> None:
        ...
        want = int(words[0])
        walked = self.climbed
        while len(walked) <= want:
            above = read_up(walked[-1])
            if above is None:
                return
            self.graph.link(walked[-1], above)
            walked = self.climbed
        self.enter(walked[want])

    def enter(self, reading: Reading, kind: str = "read") -> None:
        ...
        """
        reading = self.graph.enter(reading, kind)
        if reading is self.reading:
            return
        self.reading = reading
        self.typed.clear()
        self.main = ChainMap(
            self.rooms.setdefault(self.graph.focus, {}),
            self.policy,
        )
        self.said = None
        self.at = 0.0
    ...
    def _ring(self, _words: list[str]) -> None:
        ...
        snapshot = {k: v for k, v in self.main.items() if v}
        # Entering the ring casts the current effective instrument into its
        # session-wide policy. The policy room therefore reads the same record
        # on a second entry instead of becoming a new empty relation.
        self.policy.update(snapshot)
        held.text = record(snapshot)
        held.hold()
        self.enter(held, "policy")
    ...
    def _pin(self, words: list[str]) -> None:
        ...
        span = (
            min(covering, key=lambda held: held.end - held.start) if covering else None
        )
        internal = span.rule if span is not None else ""
        name = (
            as_written(staged(self.reading, self.main).rules, internal)
            if internal
            else ""
        )
        definition = ""
        if name:
            found = next(
                (
                    (first, last)
                    for rule, first, last in ruledefs(self.reading.reader_text)
                    if rule == name
                ),
                None,
            )
            if found is not None:
                first, last = found
                definition = "\n".join(
                    self.reading.reader_text.split("\n")[
                        first : min(last + 1, first + 3)
                    ]
                ).rstrip("\n")
        snippet = self.reading.text[s0 : min(e0, s0 + 400)]
        longest = max((len(line) for line in snippet.split("\n")), default=0)
        cell = ADVANCE["pinbody"]
        width = int(max(280, min(longest * cell + 28, self.viewport[0] * 0.62)))
        columns = max(8, int((width - 22) / cell))
        snippet_rows = sum(
            max(1, (len(line) + columns - 1) // columns) for line in snippet.split("\n")
        )
        fact = (f"field {span.field} · " if span is not None and span.field else "")
        fact += f"pinned against gen {self.generation}"
        fact_columns = max(8, int((width - 22) / ADVANCE["pinfact"]))
        fact_rows = max(1, (len(fact) + fact_columns - 1) // fact_columns)
        definition_rows = sum(
        ...
        height = int(
            max(
                ...
                + 6
                + fact_rows * 13
                + (6 + definition_rows * 14 if definition_rows else 0),
            )
        )
        wid = self.open_window("pin", words[0], width, height, pin=True)
        self.main[f"gen.{wid}"] = str(self.generation)
        self.main[f"pin.text.{wid}"] = snippet
        self.main[f"pin.rule.{wid}"] = name
        self.main[f"pin.field.{wid}"] = span.field if span is not None else ""
        self.main[f"pin.depth.{wid}"] = str(span.depth) if span is not None else ""
        self.main[f"pin.def.{wid}"] = definition
    ...
    def _rail(self, words: list[str]) -> None:
        ...
        self.open_window(
            "rail",
            words[0],
            int(min(drawn.wide + 52, self.viewport[0] * 0.72)),
            int(min(drawn.tall + 58, self.viewport[1] * 0.72)),
            pin=True,
        )
    ...
    def _railgo(self, words: list[str]) -> None:
        ...
        if not said:
            return
        facet, x, y, w, h, old = (said.split(" ", 5) + [""] * 6)[:6]
        if facet == "rail" and rule and rule != old:
            key = f"rail.hist.{wid}"
            history = self.main.get(key, "").split()
            self.main[key] = " ".join([*history, old])
            self.main[f"win.{wid}"] = f"{facet} {x} {y} {w} {h} {rule}"
            self.raise_window(wid)

    def _railback(self, words: list[str]) -> None:
        """↩ — return to the previous rule visited inside this rail window."""
        if not words:
            return
        wid = words[0]
        key = f"rail.hist.{wid}"
        history = self.main.get(key, "").split()
        said = self.main.get(f"win.{wid}", "")
        if not history or not said:
            return
        rule = history.pop()
        if history:
            self.main[key] = " ".join(history)
        else:
            self.main.pop(key, None)
        facet, x, y, w, h, _old = (said.split(" ", 5) + [""] * 6)[:6]
        if facet == "rail":
            self.main[f"win.{wid}"] = f"{facet} {x} {y} {w} {h} {rule}"
            self.raise_window(wid)
    ...
    def _reread(self, _words: list[str]) -> None:
        ...
        self.generation += 1
        previous = self.graph.focus
        self.reading = self.graph.refresh(self.reading)
        if self.graph.focus != previous:
            room = self.rooms.pop(previous, {})
            self.rooms[self.graph.focus] = room
            self.main = ChainMap(room, self.policy)
    ...
    def _save(self, _words: list[str]) -> None:
        ...
        if self.reading.reader == POLICY:
            self.policy.update(apply_record(self.reading.text))
            return
