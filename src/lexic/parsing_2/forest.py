...
- :class:`ParseTree` — ONE derivation: a non-terminal symbol over a span, with
  ...
  :class:`~lexic.parsing_2.reduce.Reducer` folds.
...
class ForestCtx(IrLeaf[IrSelf, IrSelf]):
    ...
    *mid-production* one is treated as a cycle.
    ...
    precedent as :attr:`~lexic.parsing_2.chart.Column.predicted`. Behaviour stays
...
def _forest_ctx(head: IrSelf) -> ForestCtx:
    ...
    :param head: ``nc[0]`` — a :class:`~lexic.parsing_2.chart.Chart` (top-level
...
class BuildTree(IrLeaf[IrSelf, IrSelf]):
    ...
    **raises** rather than silently picking one. It takes only the **first two**
    ...
    (potentially exponential) enumeration — so detecting ambiguity is cheap.
    """
