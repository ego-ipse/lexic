...
# ── the emit half: IR → formatted constructor notation ────────────────────
#
# The layout twin of ``repr`` and the exact inverse of ``load_ir``:
# ``load_ir(emit_ir(x)) == x``. Values are rendered by TIER (scalar leaf /
# record / variadic / plain payload), so a new node type needs no emitter
# change — the same openness the symbol table gives the parse half. Never
# emits a trailing comma (the notation's arglist does not accept one).
...
def emit_ir(node: IrSelf, width: int = 88) -> str:
    ...
    The exact inverse of :func:`load_ir` (``load_ir(emit_ir(x)) == x``) and
    the layout twin of ``repr``: same constructor syntax and default elision
    (via :meth:`~lexic.ir.base.IrNamedTuple.repr_args`), width-solved by
