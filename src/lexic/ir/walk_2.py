...
:class:`IrDispatch` is an :class:`~lexic.ir.nodes_2.IrComposite` carrying the
...
Short-circuit
-------------
A body that raises :class:`~lexic.ir.action_2.IrReturn` unwinds to the
dispatcher's catch. The IrReturn instance itself is returned — it IS-A
:class:`~lexic.ir.nodes_2.IrNode` IS-A :class:`~lexic.ir.nodes_2.IrSelf`,
fitting the dispatcher's ``Ir_co: IrSelf`` bound. Callers needing the carried
value read ``.value`` from the result.

A bare :class:`~lexic.ir.action_2._Return` (not an IrReturn) propagates past
the dispatcher.
"""
...
from lexic.ir.action_2 import IrAction, IrEmit, IrRaise, IrRebuild, IrReturn, IrWalk
from lexic.ir.nodes_2 import IrComposite, IrLiteral, IrNode, IrSelf, IrTuple
...
class IrDispatch[Ir_co: IrSelf](IrComposite[Ir_co]):
    ...
    the matching :class:`~lexic.ir.action_2.IrAction` for ``type(n)`` via
    ...
    from :class:`~lexic.ir.nodes_2.IrComposite`, so the dispatcher is not
    ...
    :param default: Body invoked when no action matches ``type(n).__mro__``.
        Base default is :class:`~lexic.ir.action_2.IrRaise`; presets
        override (e.g. :class:`IrVisitor` →
        :class:`~lexic.ir.action_2.IrWalk`).
    """
    ...
    def eval(self, d: IrSelf, n: IrSelf, nc: Sequence[IrSelf], /) -> Ir_co:
        ...
        :returns: The action body's ``Ir_co`` result, or the
            :class:`~lexic.ir.action_2.IrReturn` instance if a body raised
            one.
    ...
    def apply(self, root: IrNode) -> Ir_co:
        ...
        Catches :class:`~lexic.ir.action_2.IrReturn` from bodies and
        surfaces its ``.value`` when it satisfies the dispatcher's
        ``Ir_co`` bound. If the value does not satisfy the bound, the
        :class:`~lexic.ir.action_2.IrReturn` instance itself is checked;
        if that too does not satisfy, the exception re-raises.

        A bare :class:`~lexic.ir.action_2._Return` (not
        :class:`~lexic.ir.action_2.IrReturn`) propagates unconditionally.
    ...
    def _resolve(self, node_type: type) -> IrAction[Ir_co]:
        ...
        On a miss, constructs a synthetic :class:`~lexic.ir.action_2.IrAction`
...
class IrVisitor(IrDispatch):
    ...
    via :class:`~lexic.ir.action_2.IrWalk`, and returns
    :data:`~lexic.ir.nodes_2.IrNone`. User actions resolve first via MRO
    ...
    (e.g. :class:`~lexic.ir.action_2.IrPass`,
    :class:`~lexic.ir.action_2.IrReturn`).
    """
...
class IrTransformer(IrDispatch[IrNode]):
    ...
    the node — :class:`~lexic.ir.action_2.IrRebuild` is the canonical body.
    User actions resolve first; matching actions can return any
    :class:`~lexic.ir.nodes_2.IrNode`.
    """
...
class IrEmitter[Ir_co: IrLiteral](IrDispatch[Ir_co]):
    """Produces :class:`~lexic.ir.nodes_2.IrLiteral`-wrapped strings.
    ``Ir_co = IrLiteral``.

    Default body is :class:`~lexic.ir.action_2.IrEmit` — unmatched nodes
