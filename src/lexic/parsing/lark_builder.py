"""LarkBuilder — NewRuleSpec list (IrItem-shape) → Lark grammar + Transformer.

Replaces the legacy codegen/lark_builder.py at cutover (Slice 4).

Decision CQ #2: non-semantic optionality is driven by NewRuleSpec.non_semantic_fields
...
def _quant(q: Quantifier) -> str:
    """Convert a Quantifier to a Lark quantifier string."""
...
class LarkBuilder:
    ...
    def __init__(
        self, specs: list[NewRuleSpec], *, start_rule: str | None = None
    ) -> None:
        self.specs = specs
    ...
    def _emit_rule(self, spec: NewRuleSpec) -> str:
        """Build a Lark rule from a NewRuleSpec."""
        name = to_lark_name(spec.rule_name)
        body = self._emit_body(spec)
        return f"{name}: {body}"

    def _emit_body(self, spec: NewRuleSpec) -> str:
        """Build a Lark body from a NewRuleSpec."""
        if not spec.items:
            return '""'
...
def build_lark(
    specs: list[NewRuleSpec], classes: dict[str, type], start_rule: str
