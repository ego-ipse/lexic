...
class ForestCtx(IrLeaf[IrSelf, IrSelf]):
    ...
    :class:`~lexic.parsing.earley.kernel.loop.state.KernelState`): :attr:`open` holds the
    ...
    *mid-production* one is treated as a cycle.
    ...
    precedent as :attr:`~lexic.parsing.earley.kernel.forest.chart.Column.predicted`. Behaviour stays
...
def _forest_ctx(head: IrSelf) -> ForestCtx:
    ...
    :param head: ``nc[0]`` — a :class:`~lexic.parsing.earley.kernel.forest.chart.Chart` (top-level
...
class BuildTree(IrLeaf[IrSelf, IrSelf]):
    ...
    **raises** rather than silently picking one.
    ...
    unambiguous case never reaches it — :class:`~lexic.parsing.earley.kernel.loop.kernel.FastTree`
