...
The structured (folding-aware) tail over :mod:`lexic.parsing.pda.scanner`:
...
"rulename … defined-as" shape) :class:`~lexic.parsing.pda.scanner.ScanGate`,
or ``None`` when the decision does not separate (the loop stays an island).

A leaf w.r.t. :mod:`lexic.parsing.pda.analysis` (the kwindow/noise precedent):
...
FOLLOW table from :mod:`lexic.parsing.pda.noise`
(:func:`~lexic.parsing.pda.noise.sem_follow_table`).
"""
...
from lexic.parsing.pda.noise import sem_follow_table
...
def _sem_follow_clear(
    ...
    the rule as *semantic* content (:func:`~lexic.parsing.pda.noise
