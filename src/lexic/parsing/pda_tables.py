...
The spec representation stays flat (tuple-coded NamedTuples, no closures, no
nested dict lookups on what becomes the runtime hot path) so Task 8 can flatten
it to int-coded arrays without a shape change.
"""
...
class PdaTables(IrLeaf[IrSelf, IrSelf]):
    ...
    __slots__ = ("clones", "start_key", "islands", "instance_grammar", "_island_tables")
    ...
    def __init__(
        ...
        """Freeze the clone table and seed an empty island-tables cache."""
        self.clones = clones
