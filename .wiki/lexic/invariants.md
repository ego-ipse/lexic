...
**Round-trip fidelity.** `parse(text, grammar).to_text() == text` on every grammar-valid input. Property tests on all seven ground-truth grammars stay green.

**No regression.** The full test suite (743 tests + 1 xfail) stays green after every change.
...
## Seven ground-truth grammars

These live in `resources/ground_truth/`. All integration and property tests run against all seven. A change that makes any one of them fail is a blocker regardless of unit-test status:

`arithmetic`, `c`, `chess`, `japanese`, `json_arr`, `json_ws`, `list`

## What these invariants mean in practice

- You cannot add a new atom type without updating every dispatch table in `codegen/`, `grammars/gbnf/emitter.py`, `grammars/new_gbnf/emitter.py`, `codegen/transformer/`.
