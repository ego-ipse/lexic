...
## A gate stored as a string refuses the end of input itself

Every gate is asked about `text[pos:pos + 1]`, which is `""` past the last
character. While a gate's characters were a `frozenset`, the refusal belonged
to the CONTAINER — `"" in frozenset(...)` is False, and the read never had to
say so. Where a gate is stored as one `str` instead, `"" in "abc"` is **True**,
so the read must refuse the sentinel explicitly or every positive gate opens at
the final column and admits a derivation the grammar does not have.

Storing it as a string is a deliberate collection decision, not a micro-tweak:
the collector walks a frozenset for the life of the process and never walks a
string, and a compiled artefact holds thousands of them. The Earley seed gates
are spelled this way (`SeedGate` beside `Charset` names the difference, so the
annotation says which contract a reader is holding). The PDA's flat gates are
**not**, and that is also deliberate — they are lowered out of a `CharSet` that
outlives them, so the string would free nothing and only spread the trap.

The rule is the read's, not the type's: any gate whose container admits `""`
tests for the sentinel before the membership test.
