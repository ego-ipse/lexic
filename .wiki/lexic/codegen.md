...
- **`relax_non_semantic(ast)`** — `min=0` on every **arm-level** ref to a **nullable** rule named in `ast.non_semantic` (refs inside inline groups keep their bounds). The nullability condition is what makes this rewrite language-preserving like the other two: once `ε ∈ L(N)`, `L(N)^{lo..hi} == L(N)^{0..hi}`, so dropping the bound changes only whether the model carries an empty noise node or an absence. Over a **non-nullable** noise rule the same rewrite strictly widens the accepted language, and a widening can make an unambiguous formulation ambiguous — GBNF's `n ::= nunit+` is the case that forced the narrowing: relaxing it deleted the mandatory-separator discipline `grammars/gbnf.py` engineers maximal munch with, after which `a ::= bc` meant either one rule reference or two. Nullability is solved on the **incoming** grammar; solving it on the result would let the pass bootstrap its own licence (relaxing `n ::= nunit+` to `nunit*` makes `n` nullable, which would then license relaxing every ref to it). The fixpoint is `nullable_names`, exported from `lexic.parsing` — see [[decisions]].
