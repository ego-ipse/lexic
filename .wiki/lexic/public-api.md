...
### `generate(rule_name, rules, *, rng=None, max_depth=5, size=None)` — `generate/` (re-exported from `lexic`)
...
The opt-out is a **resolver, not a flag**: `parse_model(grammar, text, binding, ParseConfig(resolve=...))` takes a deterministic `Resolver` inside the parse's configuration, handed the derivation in hand and the witness that differs, and whatever it returns is the parse. Its behaviour is the caller's concern, not the engine's. The same resolver reaches whichever engine ends up choosing, so the answer does not depend on which route ran. `CompiledGrammar.parse(text, resolve=..., decide=...)` surfaces it too, building the `ParseConfig` itself, and reaches the **token** route as well as the char one — the promise does not depend on whether a grammar's terminals happen to name an encoding.
...
- **`watch(tables, text, executor, *, cap, config)` → `WatchedRun`** — the watched run: an ordered `Trace` of `TraceEvent(order, kind, rule, verdict, span)`, where `kind` is one of `TRACE_KINDS` (`scan` / `probe` / `rollback` / `gate`) and `span` is an `IrSpan` — the SAME record an emission's extents carry, so a trace row and a document occurrence co-select without translation. **Pay to watch**: watching re-executes the parse, and the product carries no model precisely so the re-run cannot be confused with the parse a caller already holds. It carries `cap`/`capped` instead (a truncated account says so) and `derived` — a refused predictive run is ordinary (the compile seam retries on the gated engine) and comes back as a stream ending in the refusal rather than as an exception. The unwatched path pays nothing: the instrumentation is a subclass, nothing under `parsing/pda/` imports it, and a unit gate reads `PdaKernel`'s own code objects to prove no method of it so much as names the watch.
- **`GrammarAnalysis`** — the decision taxonomy (verdicts, gate specs) the analysis produces per grammar.
- **`Decider` / `LEFTMOST_LONGEST`** — which carving of one span a parse keeps, as a VALUE beside `Resolver`: one question, `rank(carving)`, a total preorder with the maximum kept, and `grants`, the licence kinds proven for it (a predictive shortcut that commits the split answer without asking is sound only under a decider it is licensed for). Equal by kind and every field, so it can key a cached product. `LEFTMOST_LONGEST` (the first slot takes as much as it can) is the one configuration; it grants `SPLIT_GREEDY`, `ATTEMPT`, `STOP_SET`, `NOISE_GREEDY` and `GREEDY_SPLIT`, and withholds `GREEDY_ARM` until it is proven. `STOP_SET` holds because the PDA analysis files a stop-set only where its first exit is this order's answer: the loop runs longest, no text continues both ways two characters deep, or every carving builds one model.
- **`ParseConfig` / `DEFAULT_CONFIG`** — the parse's configuration as one value: `resolve` (a `Resolver`, or `IrNone` to refuse an ambiguity) and `decide` (a `Decider`, `LEFTMOST_LONGEST` by default). The two travel together through every route — `parse_model`, `earley_model`, `token_model`, `pda_model` and its islands, and the parallel split — so both engines answer one question the same way. `CompiledGrammar.parse(text, resolve=None, cores=AUTO, decide=LEFTMOST_LONGEST)` builds it from its keywords.
