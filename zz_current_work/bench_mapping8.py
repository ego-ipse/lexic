"""Benchmark: mapping.py (original) vs mapping8.py (common-ancestor, slot reads).

Run:  uv run python zz_current_work/bench_mapping8.py
...
rn = bench("IrMap8  (slots)", lambda: None, _read_map_new)
print(f"  IrMap8 vs dict floor: x{rn / r0:.2f}")
...
mn = bench("IrMultiMap8 (live bucket)", _setup_mm_new, _read_mm)
print(f"  IrMultiMap8 vs dict floor: x{mn / m0:.2f}")
...
bench("IrMultiMap8 (slots)", lambda: None, _ins_new)
