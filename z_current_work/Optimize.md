...
> **OUTCOME (2026-07-03).** Landed in full (Task 5 killed, Task 6 subsumed by
> 6R). Full-grammar product on the disputed corpus: **25.6 → 17.25 µs/char**
> (−33%; **0.57× Lark** full-vs-full, was parity); recognize ~8.0; self-emit
> product 17.3 (0.57×). Instance path: arithmetic kernel −78% via run
> collapse. Suite 1364/0, `tools/run_checks.sh` clean, official baseline
> saved (`tools/benchmark/bench_baseline.json`). Open items carried forward:
> GBNF emit-self-reparse fixpoint gap (pre-existing, unasserted); `models.py`
> wrapper diet + `utils/quantifiers.py` cleanup (punted to the codegen
> refactor); open-set derive rework is the next major effort.
...
- [x] Task 6R DONE (opus «task3-irast» reused, Option B polarity per user ruling): (1) IrNamedTuple.__repr__ omits the TRAILING run of default-valued fields (`==` vs declared default, stop at first non-default/no-default; still valid codegen — IrItem(IrLiteral('a'), IrQuantifier(1,1)) → IrItem(IrLiteral('a'))); landed first. (2) IrRule.semantic: bool = True (noise → semantic=False); IrRule.__eq__/__ne__/__hash__ exclude it (compile-channel rationale relocated from IrAst; fixpoint preserved). IrAst DROPPED the non_semantic field + REMOVED the Task-3 eq/hash override (plain (rules,start) tuple eq composes IrRule.__eq__); IrAst.non_semantic is now a derived @property (names where not r.semantic). hoist_helpers preserves semantic. (3) parse_directives DISSOLVED → private compile._scan_directives; parsing/directives.py + tests/unit/lexic/parsing/test_directives.py DELETED (Task-6 move subsumed). compile_grammar rebuilds named rules semantic=False. Flavours flag noise rules semantic=False individually (18 ABNF + 2 GBNF; non_semantic= IrAst kwargs gone); NOISE derives from GRAMMAR.non_semantic property. Undefined-rule directive silently ignored (observably identical). Fixpoints green (ABNF+GBNF equivalence 86/0); suite 1343/0 + 10 EXPECTED failures (6 repr goldens: test_action irfield/irbuild/irpipe, test_nodes irsequence/tuple/irast; 4 IrAst-3-field: test_json ×3, test_derive marks_non_semantic — all use removed non_semantic= kwarg). CLAUDE.md §IR-types/§Directives/pipeline + wiki (ir-shapes/flavour-system/architecture/public-api/log) updated. Gates clean ×6 src files. Staged.
- [x] Sonnet port wave DONE (sonnet «port-wave-6r», session 2): all 10 expected 6R failures fixed — 6 repr goldens updated to the trailing-default-omitting form, 4 IrAst-3-field constructions ported to IrRule(…, semantic=False); deleted test_directives.py's behavioral assertions re-homed as compile-level _scan_directives tests in test_compile.py (originals recovered from 3d9e35d). Suite 1364/0, ruff clean.
- [x] tools/run_checks.sh gate GREEN (coordinator, user ruling: gate for done): two cross-file R0801 duplicates fixed at root. (1) src: the memoised-collapse skeleton shared by reduce.collapsed_tables / models.collapsed_instance_tables / lexruns.recognition_tables extracted to lexruns.collapse_runs(grammar, run_mode) — grammar-side core shared, licence injected as the policy half, memo lines stay per-caller; all three callers thinned, imports pruned. (2) tests: test_derive's thrice-repeated 9-line [0-9]+ rule construction → local _charclass_plus_rule helper. Suite 1364/0; sanity+ruff+pyright+pylint EXIT=0.
- [x] Official post-effort baseline SAVED (quiet tree, --save → tools/benchmark/bench_baseline.json): subset-920 ×4 product 17.25 µs/char (pre-effort 25.6, −33%) = 0.57× lark (63.5 vs 112.4 ms); self-emit ×1 product 17.27 µs/char = 0.57× lark (34.7 vs 61.0 ms); recognize ~8.0 µs/char. Headline gate (17.0–17.5) met.
- [x] Consolidation DONE (coordinator): OUTCOME note atop this plan; Disputed.md status closed (claims verified, fix landed); README refreshed (Lark-era pipeline/flavour/architecture text rewritten for the native Earley engine, bench headline added, test count fixed); wiki log entry for the perf round; CLAUDE.md test count refreshed.
