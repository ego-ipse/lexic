...
def report_table(verdicts: Sequence[Verdict]) -> None:
    """Print every row, its interval, and the envelope it was judged against.

    Shared by every tier that publishes verdicts, so two tiers cannot drift
    into two column layouts for the same record.
    """
