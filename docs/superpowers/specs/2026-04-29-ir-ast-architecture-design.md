...
**One type family.** The IR AST leaves *are* the `RuleSpec` field types — there is no separate `*Atom` family. The existing types in `ir/atoms.py` (`LiteralAtom`, `CharClassAtom`, `RuleRefAtom`) are renamed and moved into `ir/nodes.py` as `IrLiteral`, `IrCharClass`, `IrRuleRef`. Other existing atom types (`QuantifiedLiteralAtom`, `InlineAlternationAtom`, `InlineRegexAtom`, `AlternationAtom`) are deleted outright. Quantifier travels on `IrItem` everywhere.

**`RuleSpec.items` typing.** `list[IrItem | IrAlternation]`. The `IrAlternation` admission is narrow: it appears only when `kind == "value_str"` and the rule has multiple non-empty arms (e.g. `op ::= "+" | "-"`). In that case `RuleSpec.items = [IrAlternation(arms=...)]` — the alternation is placed directly in items, NOT wrapped in `IrGroup`. Emitters dispatch on `isinstance(items[0], IrAlternation)` and render arms with the flavour's alternation separator (no parenthesisation). Single-arm value_str rules and all sequence/alternation rules use `list[IrItem]` only. (See plan Task 8 for the full convention table.)

Rationale for the `IrAlternation` admission: `IrGroup` semantically means "parenthesised group" in source; rendering a multi-arm value_str body via `IrGroup` would emit `("+" | "-")` instead of `"+" | "-"`. The `IrAlternation`-direct shape preserves the structural distinction and removes the special-case dispatch in emitters.
